...
def run(params):
    ...
    if params.input_files is not None:
        ...
        files = aares.datafiles.DataFilesCarrier(file_phil=params.input_files,
                                                 mainphil=aares.integrate.phil_core)
    ...
    if params.input.mask is not None:
        aares.my_print('Reading mask from {}...'.format(params.input.mask))
        frame_mask = aares.mask.read_mask_from_image(params.input.mask, channel='A', invert=False)
        masks = [frame_mask] * len(files.file_groups)
    else:
    ...
    px_used, px_total = aares.mask.count_used_pixels(masks[0])
    ...
    for mask, det_q, group in zip(masks, det_qs, files.file_groups):
        ...
        if normalize_beam:
            beam_bin_mask = aares.integrate.beam_bin_mask(
        ...
        group_prefix = os.path.join(params.sec.output.directory,
                                    params.sec.output.prefix + "_" + str(group_id).zfill(
                                        group_digit) + "_")
        ...
        if not os.path.isdir(params.sec.output.directory):
            try:
                ...
                raise aares.RuntimeErrorUser(
                    'Path is not a directory: {}'.format(params.sec.output.directory))
...
class JobSec(aares.Job):
    ...
    def __process_unhandled__(self):
        for param in self.unhandled:  # TODO: This  is copy from aares.integrate.JobReduction
            if aares.datafiles.is_fls(param):
                ...
                elif aares.integrate.ReductionBins.is_type(param):
                ...
                self.params.to_import.search_string.append(param)
